...
struct netif *lwip_arp_filter_netif(struct pbuf *p, struct netif *netif, u16_t type) {
    ...
    switch (type) {
        ...
        case ETHTYPE_IP: {
            ...
            for (int i = 0; i < ra_eth_dev->config.virtual_num; i++) {
                if (netif_is_up(ra_eth_dev->virt_netif[i])) {
                    if (ip4_addr_cmp(&dipaddr, netif_ip4_addr(ra_eth_dev->virt_netif[i]))) {
                        return ra_eth_dev->virt_netif[i];
                    }
                }
            }
        } break;
...
struct netif *lwip_hook_ip4_route_src(const ip4_addr_t *src, const ip4_addr_t *dest) {
    ...
    struct netif *netif = ra_eth_dev->netif;
